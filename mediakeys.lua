...
local sink_input_id = io.popen("wpctl status | awk '/Streams:/{s=1} s && /audio-src/{print $1; exit}'"):read("*l")
local music_sink = sink_input_id
local set_music_volume = "wpctl set-volume " .. music_sink
-- local set_music_volume = pactl set-sink-input-volume $musicSink
local get_music_volume = "wpctl get-volume " .. music_sink
